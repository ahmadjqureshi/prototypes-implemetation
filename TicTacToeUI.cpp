	...
	case WM_COMMAND:
		...
		if ( wmEvent == STN_CLICKED)
		{
			if (wmId >= 300 && wmId <=309 )
			{
				...
				if ( currentNode.getWinningStatePlayer1())
				{
...
Board getBoard()
{
	...
	for ( i = 0 ; i < 9; i++ )
	{
		count = SendMessage(handles[i], WM_GETTEXT, 4, (LPARAM) param);
		if ( param[0] == 'O' )
		{
			board.setValue( i, Player1 );
		}
		else if ( param[0] == 'X' )
		{
			board.setValue( i, Player2 );
		}
	}
...
void setBoard( Board board)
{
	...
	for ( i = 0 ; i < 9; i++ )
	{
		BoardSquare player = board.getValue( i );
...
void makeCPUMove()
{
	...
	int			msgBoxResult;
	Node		rootNode( board, Player1Turn );
	Children	child[8];
	Decision	paths;
	...
	moves.generateMoves( Player2Turn, &rootNode, child, turns);
	...
	paths.findBestPaths( &rootNode, i, &bestNode);

	setBoard( bestNode);
	if (turns >	5 )
		turns--;

	if ( bestNode.getWinningStatePlayer1())
	{
		msgBoxResult = MessageBox( NULL, "You win!!! Do you want to start a new game", "", MB_YESNO);
		if ( msgBoxResult == IDYES)
		{
	...
	else if ( bestNode.getWinningStatePlayer2())
	{
